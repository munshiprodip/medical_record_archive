            ...
            <div class="offcanvas-body my-auto mx-0 flex-grow-0">
                <form id="add_data_form" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="speciality_id" type="text" class="form-control">
...
@section('script')
    <script>
        ...
        function editData(id) {
            $.ajax({
                url: `{{ route('doctors.find', ':id' ) }}`.replace(':id', id),
                type: "GET",
                success: function ( res ) {
                    if (res.success) {
                        ...
                        edit_data_form.find("select[name='speciality_id']").val(res.data.speciality_id).trigger('change');
        ...
        edit_data_form.on('submit', function(e) {
            if (!e.isDefaultPrevented()) {
                ...
                $.ajax({
                    url: `{{ route('doctors.update', ':id') }}`.replace(':id', id),
                    type: "POST",
        ...
        function changeStatus(id) {
            Swal.fire({
                ...
                if(t.value){
                    $.ajax({
                        url: `{{route('doctors.change_status', ':id')}}`.replace(':id', id),
                        type: 'GET',
                        success: function(data) {
                            if (data.success === true) {
                                data_table.ajax.reload();
        ...
        function deleteData(id) {
            Swal.fire({
                ...
                if(t.value){
                    $.ajax({
                        url: `{{route('doctors.destroy', ':id')}}`.replace(':id', id),
                        type: 'POST',
                        data: {'_method': 'DELETE',},
                        success: function(data) {
                            if (data.success === true) {
